...
export default function EmailConfirmationPage() {
  ...
  const handleNext = async (e) => {
    ...
    // ✅ Lógica de recuperación/envío de ID
    let existingId = localStorage.getItem('etaIlId');
    
    const sendData = async (targetId) => {
      const body = { applicationId: targetId, ...stepPayload };
      return await fetch('/api/save-application', {
        method: 'POST',
    ...
    try {
      let res = await sendData(existingId);
      let json = await res.json();

      // ✅ FIX "Application not found": 
      // Si el ID guardado no existe en DB, borramos el ID local y re-intentamos como solicitud nueva
      if (!res.ok && (json.error?.includes("not found") || res.status === 404)) {
        console.warn("ID viejo detectado y no encontrado en DB. Reintentando como nueva solicitud...");
        localStorage.removeItem('etaIlId');
        res = await sendData(null); // Reintento sin ID
        json = await res.json();
      }

      if (res.ok && json.id) {
        localStorage.setItem('etaIlId', json.id);
        router.push('/apply/step-2');
      } else {
        alert("Error: " + (json.error || "No se pudo iniciar la solicitud."));
      }
    } catch (error) {
      console.error("Error saving step 1:", error);
      alert("Error de conexión. Se guardó localmente.");
      router.push('/apply/step-2');
    } finally {
      setLoading(false);
    }
            ...
            <div className="mb-8 p-6 bg-gray-50 rounded-xl border border-gray-200 shadow-inner">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                <div>
                  ...
                  {/* ✅ Fix: Fallback || '' evita el error de uncontrolled input */}
