...
export default function Step1() {
  ...
  const [acceptDisclaimer2, setAcceptDisclaimer2] = useState(false)

  const allAccepted = acceptDisclaimer1 && acceptDisclaimer2
  ...
  return (
    <main className="min-h-screen bg-white text-gray-800 font-sans flex flex-col">
      ...
      {/* Disclaimers */}
      <form className="flex-1 flex flex-col justify-center items-center px-4" onSubmit={handleNext}>
          ...
          <div className="space-y-6">
            <div>
              <input
                ...
                required
              />
              <label htmlFor="disclaimer1" className="text-base font-medium cursor-pointer select-none">
                <span className="font-bold">Acepto:</span> Debe obtener una Autorización Electrónica de Viaje (ETA-IL) antes de comprar cualquier pasaje a Israel. El Estado de Israel no se responsabiliza por daños derivados de la denegación o cancelación de la ETA-IL. Al enviar su solicitud, autoriza la transferencia de su información entre organismos públicos de Israel para fines migratorios. Si existen antecedentes penales, investigaciones, o prohibiciones migratorias, su solicitud podrá ser denegada o cancelada en cualquier momento. Toda la información brindada debe ser verdadera y exacta; proveer datos falsos puede resultar en sanciones legales. La ETA-IL solo permite llegar al control fronterizo y no garantiza el ingreso a Israel, quedando sujeto a la decisión del oficial en frontera.
              </label>
            </div>
            <div>
              <input
                id="disclaimer2"
                type="checkbox"
                checked={acceptDisclaimer2}
                onChange={e => setAcceptDisclaimer2(e.target.checked)}
                className="mr-3 accent-blue-950 w-5 h-5"
                required
              />
              <label htmlFor="disclaimer2" className="text-base font-medium cursor-pointer select-none">
                <span className="font-bold">Acepto:</span> Este sitio no pertenece al gobierno de Israel. Brindamos asistencia independiente para gestionar tu solicitud. La información proporcionada será tratada de manera confidencial y utilizada exclusivamente para fines de trámite migratorio.
              </label>
            </div>
          </div>
        </div>
        ...
        <button
          ...
          className={`mt-10 px-8 py-3 rounded-full font-bold text-lg shadow-md transition bg-blue-950 text-white ${
            allAccepted ? 'hover:bg-blue-800' : 'opacity-60 cursor-not-allowed'
          }`}
        >
          Siguiente paso
        </button>
